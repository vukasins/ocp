...
<input type="hidden" name="search_field_query_<?=$field->field_name?>" value="(if(:<?=$field->field_name?>_from != '', <?=$field->field_name?> >= UNIX_TIMESTAMP(:<?=$field->field_name?>_from), '1=1')) AND (if(:<?=$field->field_name?>_to != '', <?=$field->field_name?> <= UNIX_TIMESTAMP(:<?=$field->field_name?>_to), '1=1'))" />